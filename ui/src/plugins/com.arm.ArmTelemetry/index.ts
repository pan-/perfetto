...
class MetricCounterTrack extends CounterTrack {
  constructor(
    trace: Trace,
    uri: string,
    metricName: string,
    cpu: number,
    options?: Partial<Omit<CounterTrackAttrs, 'trace' | 'uri' | 'sqlSource'>>,
  ) {
    super({
      trace,
      uri,
      sqlSource: `select * from ${metricName} where cpu = ${cpu}`,
      ...options,
    });
  }
}
...
export default class ArmTelemetryPlugin implements PerfettoPlugin {
  ...
  async onTraceLoad(trace: Trace): Promise<void> {
    ...
    const telemetrySpecManager = ArmTelemetryPlugin.manager;
  ...
  //    - dur: duration of the slice
  //    - utid: Thread identifier associated with the slice.
  private async createCpuActivityViews() {
    if (!this._ctx) {
      throw new Error(
  ...
  private async updateMetricTracks(): Promise<void> {
    ...
    const restartUpdate = () => {
      ...
      if (this._requireUpdate) {
        // When we bail out we should remove the trash
        setTimeout(() => this.updateMetricTracks(), 0);
      }
    };
  ...
  private async getCpuIdMap(): Promise<Map<number, string>> {
